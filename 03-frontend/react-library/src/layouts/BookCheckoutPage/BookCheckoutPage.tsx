...
export const BookCheckoutPage = () => {
  ...
  async function submitReview(starInput: number, requestDescription: string) {
    let bookId: number = 0;
    if (book?.id) {
      bookId = book.id;
    }

    const reviewRequestModel = new ReviewRequestModel(starInput, bookId, requestDescription);
    const url = `http://localhost:8080/api/reivews/secure`;
    const requestOptions = {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
